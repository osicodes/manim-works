# https://www.youtube.com/watch?v=j449Isga0Es
from manim import *

class Sqrt2(VGroup):
    def __init__(self, n, **kwargs):
        super().__init__(**kwargs)
        body = MathTex ("\\frac{\\sqrt{%s}}{2}"%n)[0]
        number = body[2]
        self.top = body[:3]
        body.remove(body[2])
        self.add(body,number)
        
class TrigMnemonics(Scene):
    def construct(self):
        # MOBS DEFINIFTIONS
        l_buff = 1.2
        left_labels = VGroup(*[
            MathTex(t) for t in ["Radians:","Degrees:","sin","cos","tan"]
        ])
        left_labels.arrange(DOWN,buff=l_buff) # arrange elements in group from top to down
        radians_grp = VGroup(*[
            MathTex(t) for t in ["0",*["\\frac{\\pi}{%s}"%n for n in [6,4,3,2]]]
        ])
        radians_grp.arrange(RIGHT,buff=l_buff)  # arrange elements in group from left to right
        radians_grp.next_to(left_labels[0],RIGHT,buff=l_buff) # elements should be next to first element of left_labels
        degrees_grp = VGroup(*[
            MathTex(f"{t}^\\circ") for t in [0,30,45,60,90]
        ])
        # TRIG PRE
        sin_vals = VGroup(*[
            Sqrt2(n) for n in range(5)
        ])
        cos_vals = sin_vals.copy() # make unique copy of sine and attach to cosine
        cos_vals = cos_vals[::-1] # reverse cosine order
        tan_vals = VGroup(*[
            MathTex("\\frac{\\sqrt{%s}}{\\sqrt{%s}}"%(n,d))[0]
            for n,d in zip(range(5),list(range(5))[::-1])
        ])
        # TRIG POST
        sin_vals_p = VGroup(*[
            MathTex(t)[0] for t in ["0","\\frac{1}{2}","\\frac{\\sqrt{2}}{2}","\\frac{\\sqrt{3}}{2}","1"]
        ])
        cos_vals_p = VGroup(*[
            MathTex(t)[0] for t in ["0","\\frac{1}{2}","\\frac{\\sqrt{2}}{2}","\\frac{\\sqrt{3}}{2}","1"][::-1]
        ])
        tan_vals_p = VGroup(*[
            MathTex(t)[0] for t in ["0","\\frac{\\sqrt{3}}{3}","1","\\sqrt{3}","\\infty"]
        ])
        ...
        ) # change values in sin, cos and tan rows
        self.wait(3)
